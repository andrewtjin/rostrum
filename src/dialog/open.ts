...
export function openWorkspaceDialog(featureId: string): void {
  ...
  // Build from the ORIGIN so it resolves to /dialog.html regardless of the caller's page path.
  const url = `${window.location.origin}/dialog.html#${encodeURIComponent(featureId)}`;
